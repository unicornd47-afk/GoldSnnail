//! Split N-MNIST No-Replay Continual Learning
//!
//! Trains a SINGLE 16D ProjectionLayer sequentially on 3 tasks WITHOUT replay:
//!   Task 1: Digits 0, 1, 2
//!   Task 2: Digits 3, 4, 5
...
//!
//! This is the no-replay baseline for continual learning: one model, many tasks,
//! no experience replay from previous tasks.
//!
//! Run: cargo run --example nmnis_t_no_replay --release
//!
...
fn main() {
    ...
    for (task_idx, task_digits) in tasks.iter().enumerate() {
        ...
        let task_train: Vec<NmnistSample> = dataset
            .train
            .iter()
        ...
        for (digit, acc) in &current_accuracies {
            let icon = if *acc >= 0.6 { "OK" } else if *acc >= 0.3 { "WARN" } else { "FAIL" };
    ...
    println!("\n========================================");
    println!("  NO-REPLAY CONTINUAL LEARNING SUMMARY");
    println!("========================================");
    ...
    let output = format!(
        ...
        EPOCHS_PER_TASK, 0, json_tasks.join(",")
    );

    let _ = std::fs::write(
        "docs/src/development/split_nmnis_t_no_replay_results.json",
        output,
    );
    println!("\nResults exported to docs/src/development/split_nmnis_t_no_replay_results.json");
}
