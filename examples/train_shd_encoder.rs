...
fn main() {
    ...
    let dataset = ShdDataset::from_json(Path::new(&data_path))
        .expect("SHD-Daten nicht gefunden. Führe scripts/fetch_shd.sh aus.");
    ...
    for epoch in 0..epochs {
        ...
        for _ in 0..pairs_per_epoch {
            ...
            let (idx_a, idx_b) = if same_class {
                ...
                // Zwei Samples verschiedener Klassen
            ...
            let vec_a = ShdDataset::to_feature_vector(
                &dataset.train[idx_a], dataset.num_neurons, dataset.duration_ms
            );
            let vec_b = ShdDataset::to_feature_vector(
    ...
    let model_path = model_dir.join("shd_encoder_v0.1.bin");
