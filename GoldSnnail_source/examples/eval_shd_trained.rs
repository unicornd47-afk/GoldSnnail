...
fn main() {
    ...
    // Modell laden
    let model_path = std::env::var("SHD_MODEL").unwrap_or_else(|_| "models/shd_encoder_v0.1.bin".to_string());
    let model_json: serde_json::Value = serde_json::from_str(
        &std::fs::read_to_string(&model_path).expect("Modell nicht gefunden. Führe train_shd_encoder aus.")
    ).unwrap();
